...
export async function mazePreviewFromURL(url, wireframe) {
    var points = [];
    var text = ""
    await fetch(url).then((r) => r.text()).then((t) => text = t);
    console.log(text);
    var lines = text.split('\n');
    ...
    const material = new THREE.MeshStandardMaterial({
      ...
      for (var i in points) {
          if (parseInt(points[i][0]) == -1) {
              ...
              pillar.position.set(parseInt(points[i][1])*0.1, 0, parseInt(points[i][2])*0.1);
              model.add(pillar);
          }
      }
      return model;
...
export function render(doc) {
    ...
    mesh = new THREE.Mesh(new THREE.BoxGeometry(0, 0, 0), new THREE.MeshStandardMaterial());
